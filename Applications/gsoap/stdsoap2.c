...
void RprintBuf_m(const char* bufName, const unsigned char* buf, int nSize)
{
	int idx;
	Rprintf("%s: ", bufName);
...
fsend_m(struct soap *soap, const char *s, size_t n)
{
  register int nwritten, err;
  if (soap_valid_socket(soap->socket))
  {
//	RprintBuf_m("Send", s,n);
	 if (soap->ssl){
//		Rprintf("SSL\n");
	   ssl_write(&ssl,s,n);
	 } else
//		Rprintf("Not SSL\n");
	   Rprintf("Packet Size: %d\n", (int)n);

	   nwritten = GPRS_SendData(s, (int)n);
  }
  return SOAP_OK;
}

static size_t
frecv_m(struct soap *soap, char *s, size_t n)
{  int r;
  ...
  if (soap_valid_socket(soap->socket))
  {
	if (soap->ssl){
	  ...
	  if(GPRS_ReceiveData(s,&r,(soap->recv_timeout*1000)) != 0);
	}
	Rprintf("RCV Len:%d , Req Len:%d",r,n);
	RprintBuf_m("RCV", s,r);
	  if (r >= 0)
	      return (size_t)r;
...
static int tcp_gethost_m(struct soap *soap, const char *addr, char * inaddr){
  if (gethostbyname_m(addr,inaddr) == 0)     return SOAP_OK;
  return SOAP_ERR;
}

static int tcp_init_m(struct soap *soap){
  soap->errmode = 1;
  COMMS_Init();
    return 0;
}
tcp_connect_m(struct soap *soap, const char *endpoint, const char *host, int port)
{
  SOAP_SOCKET fd;
  char IP[80];
  char acport[10];
  ...
  if (soap_valid_socket(soap->socket))
   ...
   fd = 10;//socket(AF_INET, SOCK_STREAM, 0);
...
/*   DBGLOG(TEST,SOAP_MESSAGE(fdebug, "Opening socket %d to host='%s' port=%d\n", fd, host, port));
   ...
   Rprintf(" soap->host:%s, soap->port:%d, soap->path:%s\r\n",soap->host, soap->port, soap->path);

   memset(IP,0,sizeof(IP));
   sprintf(acport,"%d", soap->port);

//   if (soap->fresolve(soap, host, IP)){
////	 soap_set_sender_error(soap, tcp_error(soap), "get proxy host by name failed in tcp_connect()", SOAP_TCP_ERROR);
////	 soap->fclosesocket(soap, fd);
//	 return SOAP_INVALID_SOCKET;
//   }

   if ((COMMS_Connect((char *)soap->host,(char *)acport,(soap->connect_timeout*1000))) != 0)//"198.241.162.103"   if (connect(fd, (struct sockaddr*)&soap->peer, sizeof(soap->peer)))
   {
   ...
   if (!soap_tag_cmp(endpoint, "https:*"))
   {
	 ...
	 if (InitialzeSSL((char*)host, port)) {
	   Rprintf("SSL Failed\r\n");
	   return SOAP_INVALID_SOCKET;
	 } else {
	   soap->ssl = &ssl;
	   Rprintf("SSL succeeded\r\n");
	 }
	 soap->imode |= SOAP_ENC_SSL;
	 soap->omode |= SOAP_ENC_SSL;
   } else
...
/******************************************************************************/
tcp_disconnect_m(struct soap *soap){
  Rprintf("\ntcp_disconnect_m\n");
...
#else
  soap->fresolve = tcp_gethost_m;
  soap->fopen = tcp_connect_m;
  soap->fclose = tcp_disconnect_m;
  soap->fsend = fsend_m;
  soap->frecv = frecv_m;
...
{ register const char *s;
  ...
  if ((err = soap->fposthdr(soap, "User-Agent", "gSOAP/2.8")))
    return err;
  if ((err = soap_puthttphdr(soap, SOAP_OK, count, action)))
    return err;
#ifdef WITH_ZLIB
#ifdef WITH_GZIP
...
{ register int err;
  ...
  if ((err = soap->fposthdr(soap, "Server", "gSOAP/2.8"))
   || (err = soap_puthttphdr(soap, status, count, 0)))
    return err;
#ifdef WITH_COOKIES
...
{
  struct soap *soap = (struct soap*)MYMALLOC(sizeof(struct soap));
  if (soap)
...
{ return soap_copy_context((struct soap*)MYMALLOC(sizeof(struct soap)), soap);
}
#endif
...
int
SOAP_FMAC2
soap_puthttphdr(struct soap *soap, int status, size_t count, const char *action)
{ if (soap->status != SOAP_GET && soap->status != SOAP_DEL && soap->status != SOAP_CONNECT)
  { register const char *s = "text/xml; charset=utf-8";
    ...
    if (action != 0){
      ...
      {
    	sprintf(soap->tmpbuf, "%s;action=\"%s\"", s, action && strlen(action) < sizeof(soap->tmpbuf) - 3 ? action : SOAP_STR_EOS);
    	s = soap->tmpbuf;
      }
    }
#ifndef WITH_LEANER
