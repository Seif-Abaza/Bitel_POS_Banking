...
#include "../../../BF_SDK/INCLUDE/BTEMEA_LIB/ssl/ssl.h"

#define wchar_t int
typedef long time_t;
extern unsigned long strtoul_m(	const char *nptr, char **endptr, register int base);
extern long strtol_m( const char *nptr, char **endptr, register int base);
extern double strtod(const char *string, char **endPtr);
extern int sscanf(const char *ibuf, const char *fmt, ...);

extern int GPRS_DisConnect();
extern int GPRS_ReceiveData(unsigned char *ucpRecvBuffer,int *ipLen,unsigned short usTimeOut);
extern int GPRS_SendData(unsigned char *ucpData,int iLen);
extern unsigned char COMMS_Init();
extern unsigned char COMMS_Connect(char *cpIP,char *cpPORT,unsigned short usTimeOut);
extern unsigned char gethostbyname_m(const char* hostname, char* IP );
extern int Biteltime(int ref);
extern unsigned char InitialzeSSL(char* acIPaddress,int iPort);
extern unsigned char FreeAllSSL();
...
# define SOAP_MALLOC(soap, size) MYMALLOC(size)
#endif

#ifndef SOAP_FREE			/* use libc free */
# define SOAP_FREE(soap, ptr) MYFREE(ptr)
#endif
...
#ifdef HAVE_STRTOUL
# define soap_strtoul(s, t, b) strtoul_m(s, t, b)
#else
...
SOAP_FMAC1 int SOAP_FMAC2 soap_puthttphdr(struct soap*, int status, size_t count, const char *action);
