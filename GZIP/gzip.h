...
#ifdef MY_DEBUG
#define DBGLine_m(...)	{ Rprintf(__VA_ARGS__); \
		Rprintf("\r\n"); }
#else
#define DBGLine_m(...)
#endif
...
*/
#define INBUFSIZ 0x1000 /* input buffer size */
...
EXTERN( unsigned short, head ); /* buffer for distances, see trees.c */
...
#define put_byte(c) {outbuf[outcnt++]=(unsigned char)(c); if (outcnt==OUTBUFSIZ)\
	flush_outbuf();}
#define put_ubyte(c) {window[outcnt++]=(unsigned char)(c); if (outcnt==WSIZE)\
	flush_window();}

/* Output a 16 bit value, lsb first */
#define put_short(w) \
{ if (outcnt < OUTBUFSIZ-2) { /* compare */\
	outbuf[outcnt++] = (unsigned char) ((w) & 0xff);/* 低位 */ \
	outbuf[outcnt++] = (unsigned char) ((unsigned short)(w) >> 8);/* 低位 */ \
} else { \
	put_byte((unsigned char)((w) & 0xff)); /* 低位 */\
	put_byte((unsigned char)((unsigned short)(w) >> 8));/* 低位 */ \
} \
}
...
#define put_long(n) { \
	...
	put_short(((unsigned long)(n)) >> 16); /* 低位 */\
}
...
/* in deflate.c */
void lm_init OF(( int pack_level, unsigned short * flags ));
unsigned long deflate OF(( void ));

/* in trees.c */
void ct_init OF(( unsigned short * attr, int * method ));
int ct_tally OF(( int dist, int lc ));
unsigned long flush_block OF(( char * buf, unsigned long stored_len, int eof ));
...
/* in util.c: */
extern unsigned long updcrc OF(( unsigned char * s, unsigned n ));
extern void clear_bufs OF(( void ));
extern int fill_inbuf OF(( void ));
extern void flush_outbuf OF(( void ));
extern void flush_window OF(( void ));
extern void write_buf OF(( char * buf, unsigned cnt ));
