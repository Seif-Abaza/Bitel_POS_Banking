...
* for error return.
*/
unsigned char * zip_mem_outptr;
int zip_mem_outlen;
void write_buf( char * buf, unsigned cnt )
{
  Rprintf("write_buf\r\n");
  DBGLine_m("zip_mem_outptr:0x%08lX",zip_mem_outptr);
  if ( (zip_mem_outptr >= ((unsigned char*)0x30000000UL)) && (zip_mem_outptr < ((unsigned char*)0x40000000UL)) )
  {
	  Rprintf("FLASH_WRITE\r\n");
	FLASH_WRITE((char *)(zip_mem_outptr + zip_mem_outlen), cnt, (char *)buf);
  }
  else
  {
