...
static const unsigned short border[] = {
...
unsigned long bk = 0; /* bits in bit buffer */
...
oversubscribed set of lengths), and three if not enough memory. */
{
	...
	for ( i = BMAX; i; i --)
		if ( c[i] ){
			//Rprintf(" iiiiiiiiiiiiiiiiiiiiiii\r\n i,ci=%d %d \r\n ",i,c[i]);
			break;
		}
			...
			while ( k > w + l )
			{
				...
				{ /* too few codes for k-w bit table */
					...
					while ((++j) < z ) /* try smaller tables up to z bits */
					{
						...
						//Rprintf(" inside %d %d %d %d %d %d %d     %d %d\r\n",z,g,w,h,k,l,j,*(xp+1),f  );
						if (( f <<= 1 ) <= *++ xp )
	...
	for (;;) /* do until end of block */
	{
		...
		if (e  > 16 ) /* bits */
			do {
				...
				e -= 16; /* bits */
				NEEDBITS( e );
				//Rprintf("\r\n eee=%d %d %d %d  %d %d\r\n",e,lrt++,t->v.n,t,t->v.t,ml);
				t= t->v.t + (( unsigned short)b & mask_bits[e] );
				//Rprintf("\r\n eee=%d %d %d %d  %d %d\r\n", mask_bits[e],b,( t )->e,t,b,ml);
				e = ( t )->e;
		...
		//Rprintf("\r\n eeee=%d %d %d %d  %d %d\r\n",e,lrt++,t->v.n,w,b,ml);
		DUMPBITS( t->b );
		if (( e == 16)) //&& (t->v.n!=0) added by Mohamed Moustafa for unknown bug reason/* then it's a literal */
		{
		...
		else /* it's an EOB or a length */
		{
			...
			NEEDBITS( e );
			n = t->v.n + (( unsigned short)b & mask_bits[e] );
			DUMPBITS( e );

			/* decode distance of block to copy */
			NEEDBITS(( unsigned int)bd )
				if (( e = ( t = td + (( unsigned short)b & md ))->e ) > 16 )
					do {
						if ( e == 99 )/* comment */
							return 1;
						DUMPBITS( t->b );
						e -= 16; /* bits */
						NEEDBITS( e );
					} while (( e = ( t = t->v.t + (( unsigned short)b & mask_bits[e] ))->e ) > 16 ); /* bits */
			DUMPBITS( t->b );
			NEEDBITS( e )
				d = w - t->v.n - (( unsigned short )b & mask_bits[e] );
			DUMPBITS( e )
...
/* decompress an inflated type 2 (dynamic Huffman codes) block. */
{
	...
	bl = 7;
	tl=0;
	if (( i = huft_build( ll, 19, 19, 0, 0, & tl, & bl )) != 0 ) /* bits */
	{
		if ( i == 1 )
			huft_free( tl );
