...
#define TOO_FAR 4096 /* TOO FAR */
	   ...
	   */
	   ...
	   unsigned long window_size = ( unsigned long )2 * WSIZE;
...
#define H_SHIFT ((HASH_BITS+MIN_MATCH-1)/MIN_MATCH)
	   ...
	   */
	   ...
	   unsigned near short strstart = 0; /* start of string to insert */
	   unsigned near short match_start = 0; /* start of matching string */
	   local int eofile = 0; /* flag set at end of input file */
	   local unsigned short lookahead = 0; /* number of valid bytes ahead in window */

	   unsigned near short max_chain_length = 0;
...
#define max_insert_length max_lazy_match
	   ...
	   */
	   ...
	   unsigned near short good_match = 0;
		   ...
		   */
		   void lm_init ( int pack_level, unsigned short * flags )
		   {
			   ...
			   */
			   ...
			   DBGLine_m("pack_level: %d, max_lazy_match:%d,good_match:%d,nice_match:%d,max_chain_length:%d",pack_level, max_lazy_match,good_match,nice_match,max_chain_length);
...
#define check_match(start, match, length)
		   ...
		   */
		   local void fill_window()
		   {
			   ...
			   unsigned more = ( unsigned )( window_size - ( unsigned long )lookahead - ( unsigned long )strstart );
			   /* Amount of free space at the end of the window. */

			   DBGLine_m("fill_window");
			   ...
			   */
			   if ( more == ( unsigned )- 1 ) {
				   ...
				   */
				   Assert( window_size == ( unsigned long )2 * WSIZE, "no sliding with BIG_MEM" );
	...
	(char*)NULL, (long)strstart - block_start, (eof))
		   ...
		   */
		   unsigned long deflate()
		   {
			   ...
			   register unsigned char match_length = MIN_MATCH - 1; /* length of best match */

			   DBGLine_m("deflate 1");
			   DBGLine_m("hash_head:%d,  max_lazy_match:%d,",
			  				             hash_head, max_lazy_match);
			   ...
			   while ( lookahead != 0 ) {
				   ...
				   */
				   ...
				   DBGLine_m(" ****************************");
				   DBGLine_m(" prev_length:%d, strstart:%d",prev_length,  strstart);
				   DBGLine_m("hash_head:%d,  max_lazy_match:%d,",hash_head, max_lazy_match);
				   DBGLine_m(" lookahead:%d, strstart:%d, match_length:%d, eofile:%d", lookahead,strstart,match_length,eofile);
				   ...
				   DBGLine_m(" prev_length:%d, match_length:%d", prev_length,match_length);
