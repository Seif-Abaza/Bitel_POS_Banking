...
//#define HASH_SIZE1 (unsigned)(1<<HASH_BITS)
...
DECLARE( unsigned short, head, (unsigned)1L << 15 );

/* local variables */
int quiet = 0; /* be very quiet (-q) */
const int maxbits = BITS; /* max bits per code for LZW */
...
int zipmem( unsigned char * mem_inptr, int mem_insize, unsigned char * mem_outptr )
{
	...
	ALLOC( unsigned char, d_buf, DIST_BUFSIZE );
	ALLOC( unsigned char, window, 2L * WSIZE ); /* 分配 */
	ALLOC( unsigned short, tab_prefix, 1L << BITS );
	ALLOC( unsigned short, head, (unsigned)1L << 15 );
